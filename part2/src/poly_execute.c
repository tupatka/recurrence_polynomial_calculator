 ...
 */
static bool IsStringCoeff(char* polyS, size_t lineSize) {
 ...
 */
static int HowManyMonos(char* polyS, size_t lineSize) {
    int howMany = 1;
 ...
 */
static bool IsCorrectPoly(char* polyS, size_t lineSize) {
 ...
 * @param[in] polyS : wczytywany wielomian
 * @param[in] okPoly : czy wczytywany wielomian jest poprawny
 * @return sparsowany wspołczynnik wielomianu
 */
static poly_coeff_t ParseCoeff(char* polyS, bool* okPoly) {
    char* pEnd;
 ...
 */
static size_t IterateTillMonoEnd(size_t indStart, size_t lineSize, char* polyS) {
    size_t i = indStart;
    int numOfLeftBraces = 0;
    int numOfRightBraces = 0;
 ...
 * @param[in] polyS : napis reprezentujący wielomian
 ...
 */
static size_t FindComaBeforeExp(char* monoToParse, size_t monoSize) {
    size_t i = monoSize - 1;
    while (true) {
        if (monoToParse[i] == ',') {
            return i;
 ...
 */
static poly_exp_t ParseExp(char* expS, bool* okPoly) {
    poly_exp_t e;
 ...
 */
static size_t FindPolyAsCoeffBegin(char* monoToParse, size_t monoSize) {
    size_t i = monoSize;
    int numOfLeftBraces = 0;
    int numOfRightBraces = 0;
...
static Poly Parse(char* polyS, size_t lineSize, bool* okPoly) {
    ...
    for (int i = 0; i < howMany; i++) {
        ...
        // ustawiamy go tak żeby przed kolejnym wywołaniem wskazywal na
        // pierwszy lewy nawias nastepnego jednomianu
